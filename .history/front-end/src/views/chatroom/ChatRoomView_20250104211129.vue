...
onMounted(() => {
  if (!userId) {
   ...
   try {
    ...
    userInfo.value = res
    // 初始化WebSocket连接,传入nickname
    chatRoomStore.initWebSocket('public-room', userId, userInfo.value.nickname || userId)
  } catch (error) {
    console.error('获取用户信息失败:', error)
    chatRoomStore.initWebSocket('public-room', userId, userId)
  }
